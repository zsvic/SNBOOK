...
**内部类的初始化**：内部类创建对象时，会自动包含一个引用指向一个外部类对象，这也是为什么内部类可以访问外部类成员的原因。其创建和初始化都要依赖于外部类(非Static内部类)。
...
```
**内部类的访问权限**：内部类的访问权限不同于一般的类，而和类中成员相似，可以拥有四种访问权限，当内部类为private时，只有外部类能够访问，这样可以彻底隐藏内部类。
