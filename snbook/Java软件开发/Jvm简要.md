...
### 1.2.1 前期编译
前期编译指的是将Java源代码转换为字节码指令的过程。

### 1.2.2 即时编译
指在Jvm运行过程中，字节码指令通过解释执行，为了提高代码执行效率，有些虚拟机的会将一些重复运行的热点字节码指令编译成机器指令。
...
在了解类加载过程之前，需要了解一些Jvm的基本情况。
##### Jvm的运行时常量池
关于运行时常量池，在[1.1](##-1.1-内存管理机制)中已经介绍了Jvm的运行时数据区，常量池被分配在方法区中。
...
### 1.4.1 加载
##### 类加载器
- 引导类加载器(Bootstrap Class Loader)：由Jvm自带，负责加载JDK中自带库的加载，用户程序不能直接使用引导类加载器。
- 用户自定义类加载器(User-defined Class Loader)：继承了抽象类`java.lang.ClassLoader`，默认实现了两种类加载器，开发者可以直接使用：
    - 扩展类加载器：加载JDK的扩展库
    - 应用类加载器：也称为系统类加载器，负责加载ClassPath上的类库，应用程序一般情况下默认使用这个类加载器

##### 类加载约束
每一个类加载器都有自己独立的命名空间，两个不同类加载器加载同一个Class时，会创建两个不等的Class对象。也就是说，Class对象的唯一性是由类加载与类名称共同确定的。

##### 创建Class对象
需要加载一个class文件时，Jvm首先需要确定一个类加载器来加载该class文件，在类加载后，Jvm会依据class文件为非数组类或接口生成一个Class对象，如果Class对象有直接父类，会优先加载父类，并在解析阶段完成Class对象到直接父类Class对象的符号引用的解析工作。
> 如果是接口(interface)，它默认有一个`Object`的直接父类，所有类和接口中，只有`Object`类没有直接父类。

### 1.4.2 连接

##### 验证
验证Class文件的字节流中包含的信息符合当前虚拟机的要求。
##### 准备
为static修饰的类变量在方法区分配空间并赋"零值"。
##### 解析
将常量池中的符号引用解析到直接引用。


### 1.4.3 初始化
该阶段执行`<clinit>()`方法，`<clinit>()`方法由编译器按顺序收集类中所有类中静态成员变量与静态代码块中的语句合并产生的。

> 虚拟机会保障一个类的`<clinit>()`方法在多线程环境下只执行一次，因为类对象只需要被初始化一次。
